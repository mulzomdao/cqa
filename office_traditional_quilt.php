<!DOCTYPE html>
<html lang="en">
    
<?include "include/head.php";?>

<body>    
    <?include "include/top.php";?>

    <div class="page-titles title-dark pt20 pb10 mb30">
        <div class="container">
            <div class="row">
                <div class=" col-md-6">
                    <h4>지회/지부 - <span>실용전통퀼트</span></h4>
                </div>
                <div class=" col-md-6 mb0">
                    <ol class="breadcrumb text-md-right">
                        <li class="breadcrumb-item active"><a href="index.php">Home</a></li>
                        <li class="breadcrumb-item active">지회/지부</li>
                        <li class="breadcrumb-item">실용전통퀼트</li>
                    </ol>
                </div>
            </div>
        </div>
    </div><!--page title end-->

    <div class="container mb30" style="padding-left: 30px; padding-right: 30px;">   

        <article class="article-post mb10" style="border-bottom-width: 0px; padding-bottom: 0px">
            <a class="post-thumb mb30" href="#">
                <img src="image/1H7U2732-00-2_resize.jpg" alt="" class="img-fluid">
            </a>
        </article>
        
        <h4 class="mb10">실용전통공예 분과는</h4>
        <p>
            (유.무형)문화재 복원활용을 목적으로 고증을 거쳐 조형, 섬유, 단청분야을 적절한 복원 방법을 사용하여 학생 및 일반에게 역사교육의 장으로 그리고 문화적 체험의 장으로 실천적 방안의 개념을 도입하여 문화재의 가치를 고유성에 두고 현실적 공간에서 활용하는 쓰임의 공예로써 표현물 체험교육을 목적으로 합니다.
        </p>     
        <h4 class="mb10">교육소개</h4>
        <p>
            문화유산은 지속과 전환의 재생산과정을 거쳐 왔고, 일정범위의 공간을 필요로 하고, 공간적 차이를 가지고 발달한다는 지역 개념의 활용이 특징이라 할 수 있습니다.  
            문화재를 재현적 기능과 지속가능한 쓰임을 두고 응용 및 활용하여 공예인으로써의 역사성, 예술성, 기술성에 중점을 두고 체계적인 문화유산방문교육을 지원합니다.
        </p>     
        <h4 class="mb10">교육운영방향</h4>
            문화재 활용 및 복원을 토대로 실질적인 기능자인력 양성이 될 수 있도록 교육과정을 운영합니다.
            
            <ul>
                <li>
                    유물 및 유적 조형공예</br>
                    문화재를 입체적인 조형물로 표현하는 수업으로 직접 디자인하여 반영구적 형틀를 만들 수 있습니다.
                </li>
                <li>
                모사 섬유공예</br>
                문화재를 모사하여 다양한 섬유로 표현할 수 있는 수업으로 문화적 다양성과 창의성 발현에 기여하며 배워 볼 수 있습니다.
                </li>
                <li>
                단청 활용공예</br>
                단청에 대한 이해와 재미를 느낄 수 있는 체험 교육을 현대에 맞게 활용한 실용품을 만들 수 있습니다.
                </li>
            </ul>
            
        <h4 class="mb10">(유.무형)문화재 복원공예기능자 자격증</h4>
        <p>
            수료 및 포트폴리오 평가 후  강사 자격증이 발급됩니다.
        </p>    
    </div>

    <?include "include/bottom.php"?>

    <?include "include/js.php"?>

    <script>
        $(document).ready(function() {
            // $('#side-menu').find('li').addClass('active');

            console.log("test");
            
        });
    </script>
</body>

<!-- Mirrored from bootstraplovers.com/assan-v3.6/classic-template/html/header-light-top-bar.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 01 Mar 2018 20:06:19 GMT -->
</html>
